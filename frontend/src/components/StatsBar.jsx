export default function StatsBar({ logs, latestIncident }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 border-b border-white/5">
      {stats.map((stat) => (
        <div key={stat.label} className="group flex flex-col justify-between p-6 transition-all hover:bg-white/[0.02] border-r border-white/5 last:border-r-0">
          <div className="flex items-center justify-between mb-4">
            <span className="text-[10px] font-black tracking-[0.2em] text-zinc-500 uppercase">{stat.label}</span>
            <span className="text-zinc-600 group-hover:text-indigo-500/50 transition-colors">{stat.icon}</span>
          </div>
